...
Routine({
	...
	~wubDrums = {
		...
		Routine({
			...
			Pbind(
				...
				\dur, Pseq([0.125, 0.25, 0.125] * speed, 64),
				\pan, 1,
				\amp, 0.1
			).play;
		}).play;
	...
	8.wait;
	~breakBuf.value(\cutBuf, cbuf, 0.15, [7.5, 20, 2.5, 25], [8, 4, 3, 1], 2); // 32s
	16.wait;
	...
	8.wait;
	Synth(\scale, [\freq, 200, \fadeOut, 2, \amp, 0.05]);
	8.wait;
	Synth(\distantMachine, [\amp, 0.05, \sustain, 10, \fadeOut, 6]); // 16s
	5.wait;
	Synth(\scale, [\freq, 200, \fadeOut, 2, \amp, 0.05]);
	5.wait;
	Synth(\buzzDust, [\amp, 0.2, \fadeIn, 4, \sustain, 6, \fadeOut, 4]); // 10s
	10.wait;
	...
	8.wait;
	~wubMelody.value(\smoothWave, 10, 0.15, 1.5); // 80s
	8.wait;
	~wubDrums.value; // 96s
	8.wait;
	Synth(\twinkle, [\amp, 0.05, \sustain, 36, \fadeOut, 8]); // 36s
	...
	~wubMelody.value(\doubleSmoothWave, 6, 0.2); // 48s
	8.wait;
	~breakBuf.value(\cutBuf, cbuf, 0.2, [2, 6, 2, 15], [8, 4, 3, 1], 2); // 32s
	8.wait;
	...
	16.wait;
	~breakBuf.value(\cutBuf, hkbuf, 0.5, [2, 60, 4, 20], [8, 4, 3, 1], 1); // 16s
	~wubMelody.value(\doubleSmoothWave, 6, 0.2); // 48s
	~wubMelody.value(\smoothWave, 5, 0.15, 1.5); // 40s
	16.wait;
	~melody4.value(\doubleSmoothWave, 2, 0.15); // 16s
	16.wait;
	~melody3.value(\doubleSmoothWave, 1, 0.2, 0.5); // 16s
	8.wait;
	...
	(8.5).wait;
	~simpleSnare.value(6, 0.3, [1,0.25,0.75,1], 1, 1, [3,1,1,4]); // 48s
	(7.5).wait;
